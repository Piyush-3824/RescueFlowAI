...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-50">User Profile</h1>
        <p className="text-xs text-slate-500">Manage your account information and professional safety credentials.</p>
      </div>

      {/* Profile Header Card */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xs flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="text-center sm:text-left space-y-1">
          <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
            <h2 className="text-xl font-extrabold text-slate-900 dark:text-slate-50">{form.name}</h2>
            <span className="rounded-md bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 border border-emerald-200">
              Verified Safety Officer
            </span>
          </div>
          <p className="text-xs text-slate-500">{form.role} • {form.department}</p>
          <p className="text-xs font-semibold text-slate-700 dark:text-slate-300">📍 {form.company} — {form.location}</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 shadow-xs space-y-5">
        <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 border-b border-slate-100 dark:border-slate-800/60 pb-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/60 px-3 py-2 text-xs font-bold text-slate-900 dark:text-slate-50 focus:border-amber-500 outline-none"
              />
            </div>

            <div className="space-y-1">
              <label className="font-bold text-slate-700 dark:text-slate-300 block">Work Email</label>
              <input
                value={form.email}
            ...
            <div className="space-y-1">
              <label className="font-bold text-slate-700 dark:text-slate-300 block">Department</label>
              <input
